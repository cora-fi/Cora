/* ============================================================
   Cora — Pantalla 6: Mis solicitudes
   ============================================================ */
function Solicitudes({ go }) {
  const claims = useAsync(() => coraService.getClaims(), []);
  const [open, setOpen] = useState(null);

  return (
    <div style={{ maxWidth: 880 }}>
      <Rise delay={0.02}><SectionTitle eyebrow="El historial de tus casos"
        sub="Cada solicitud que enviaste y en qué paso va.">
        Mis solicitudes
      </SectionTitle></Rise>
            ...
            {claims.data && claims.data.map((c) => {
              ...
              return (
                <Card key={c.id} pad="0" style={{ overflow: 'hidden' }}>
                  <button onClick={() => setOpen(isOpen ? null : c.id)} style={{ width: '100%', textAlign: 'left',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                      ...
                      <div>
                        ...
                        <div className="font-mono" style={{ fontSize: 11.5, color: 'var(--ink-2)', marginTop: 2 }}>
                          {fmtDate(c.fecha)} · {c.id}
                        </div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                      <span className="font-serif tnum" style={{ fontSize: 19 }}>{money(c.montoSolicitado)}</span>
                    ...
                    <div style={{ padding: '0 var(--s3) var(--s3)', borderTop: '1px solid var(--hairline)' }}>
                      <div className="cora-claim-detail" style={{ display: 'grid', gridTemplateColumns: '1fr 1.1fr', gap: 'var(--s4)', paddingTop: 'var(--s3)' }}>
                        <div style={{ display: 'grid', gap: 14, alignContent: 'start' }}>
                          <DetailLine label="Días en lista de espera" value={`${c.diasEnLista} días`} />
                          ...
                          <DetailLine label="Monto solicitado" value={money(c.montoSolicitado)} />
                        </div>
                        <div>
...
Object.assign(window, { Solicitudes, DetailLine });
