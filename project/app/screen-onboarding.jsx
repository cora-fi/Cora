/* ============================================================
   Cora — Pantalla 1: Ingreso / Onboarding (mock Privy)
   ============================================================ */
function Onboarding({ onLogin }) {
  const [step, setStep] = useState('bienvenida'); // bienvenida | login
  const [mode, setMode] = useState('email');       // email | passkey
  ...
  const doLogin = async () => {
    ...
    setLoading(true);
    try {
      const member = await coraService.login();
      onLogin(member);
    } catch (e) {
