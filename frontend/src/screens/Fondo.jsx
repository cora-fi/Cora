...
export default function Fondo() {
  ...
  return (
    <div style={{ maxWidth: 1040 }}>
      <Rise delay={0.02}>
        <SectionTitle eyebrow="Transparencia"
      ...
      {loading ? (
        ...
        <>
              ...
              <div className="cora-fund-top" style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 'var(--s5)', alignItems: 'center' }}>
                <div>
                  ...
                  <p style={{ color: 'var(--ink-2)', marginTop: 14, lineHeight: 1.55, maxWidth: 360 }}>
                    Es la suma de los aportes de {pool.data.member_count} miembros, más el rendimiento que ha generado.
                  </p>
                </div>
                <div>
                  <div style={{ display: 'flex', height: 18, borderRadius: 99, overflow: 'hidden', marginBottom: 18 }}>
                    <div style={{ width: `${(pool.data.yield_placed / pool.data.total_reserve) * 100}%`, background: 'var(--forest)' }} />
                    <div style={{ width: `${(pool.data.available / pool.data.total_reserve) * 100}%`, background: 'var(--clay)' }} />
                  </div>
                  <CompRow color="var(--forest)" label="Generando rendimiento"    value={pool.data.yield_placed} total={pool.data.total_reserve} />
                  <CompRow color="var(--clay)"   label="Disponible para atenciones" value={pool.data.available}    total={pool.data.total_reserve} />
          ...
          <Rise delay={0.14} style={{ marginBottom: 'var(--s3)' }}>
            <div className="cora-fund-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'var(--s3)' }}>
              <Metric icon="users-three" label="Miembros activos"     value={pool.data.member_count} count color="var(--forest)" />
              <Metric icon="heartbeat"   label="Atenciones co-pagadas" value={pool.data.claims_paid}  count color="var(--clay)" />
              <Metric icon="trend-up"    label="Rendimiento acumulado" value={yld.data.yield_amount}  money count color="var(--forest)"
                foot={`${yld.data.apy_approx}% aprox. anual`} />
            </div>
          </Rise>
