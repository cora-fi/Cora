...
export function Money({ value, decimals = 0, count = false }) {
  const counted = useCountUp(value, { decimals, start: count });
  const shown = count
    ? (decimals > 0 ? counted : Number(counted.replace(/,/g, '')).toLocaleString('en-US'))
...
/* ---------- botón ---------- */
export function Button({ children, variant = 'primary', size = 'md', icon, iconRight, full, loading, disabled, onClick, type = 'button' }) {
...
/* ---------- card ---------- */
export function Card({ children, className = '', style, pad = 'var(--s3)', as: El = 'div', onClick, hover }) {
  ...
  return (
    <El onClick={onClick}
      ...
      style={{ padding: pad, transition: 'border-color .16s', borderColor: h ? '#cfc6b2' : undefined,
...
/* ---------- progreso lineal ---------- */
export function ProgressBar({ value, max, color = 'var(--forest)', height = 8, animate = true }) {
...
export function StateWrap({ status, error, onRetry, skeleton, empty, isEmpty, children }) {
...
export function Field({ label, hint, error, children }) {
  return (
    <label style={{ display: 'block' }}>
      ...
      {children}
      {error
        ? <span style={{ display: 'block', marginTop: 6, fontSize: 12.5, color: 'var(--error)' }}>{error}</span>
        : hint
          ? <span style={{ display: 'block', marginTop: 6, fontSize: 12.5, color: 'var(--ink-2)' }}>{hint}</span>
          : null}
    </label>
  );
}
