/* ============================================================
   Cora — Pantalla 5: Solicitar ayuda (claim) + línea de tiempo
   ============================================================ */
const CLAIM_STEPS = [
...
const stepIndex = (estado) => {
  ...
  const i = CLAIM_STEPS.findIndex((s) => s.key === estado);
  return i < 0 ? 0 : i;
};

function ClaimTimeline({ claim, vertical = true }) {
  const active = stepIndex(claim.estado);
  const rechazado = claim.estado === 'rechazado';
  return (
          ...
          <div key={s.key} style={{ display: 'flex', gap: 14, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <span style={{ width: 32, height: 32, borderRadius: 99, flexShrink: 0, display: 'grid', placeItems: 'center',
                ...
                {done ? <Icon name="check" size={16} weight="bold" />
                  : current && claim.estado === 'en_validacion' ? <span className="cora-spin" style={{ width: 13, height: 13, color: 'var(--forest)' }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                {current && s.key === 'en_validacion' &&
                  <span className="tnum" style={{ fontSize: 12, color: 'var(--amber)', fontWeight: 600 }}>
                    {claim.aprobaciones} de {claim.aprobacionesNecesarias}
                  </span>}
              </div>
...
function Solicitar({ member, go }) {
  ...
  if (!member.elegible) return <NoElegible member={member} go={go} />;

  const set = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const diasEn = (() => {
    if (!form.fechaLista) return null;
    const d = (new Date(2026, 5, 3) - new Date(form.fechaLista)) / 86400000;
    return Math.round(d);
  })();
  ...
  const enviar = async () => {
    ...
    setPhase('sending');
    try {
      const c = await coraService.submitClaim({
        referencia: form.referencia, diasEnLista: diasEn,
        hospitalId: form.hospitalId, monto: Number(form.monto),
      });
      setClaim(c);
      setPhase('done');
    } catch (e) { setPhase('form'); }
  };
          ...
          <Card pad="var(--s4)">
            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24,
              ...
              <div>
                ...
                <div className="font-mono" style={{ fontSize: 14, fontWeight: 600 }}>{claim.id}</div>
              </div>
              <div>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div className="font-serif tnum" style={{ fontSize: 22 }}>{money(claim.montoSolicitado)}</div>
              </div>
            </div>
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <Rise delay={0.02}><SectionTitle eyebrow="Cuando la salud no puede esperar"
        ...
        <Card pad="var(--s4)" style={{ display: 'grid', gap: 22 }}>
          ...
          <div className="cora-two-col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
            <Field label="Desde cuándo estás en lista" error={errs.fechaLista}
              ...
              <input type="date" max="2026-06-03" value={form.fechaLista} onChange={(e) => set('fechaLista', e.target.value)}
      ...
      <Rise delay={0.08}>
        <Card pad="var(--s4)" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--ink-2)', margin: '10px auto 20px', maxWidth: 420, lineHeight: 1.55 }}>
            Para poder solicitar ayuda hay que cumplir {member.mesesCarencia} meses aportando. Llevás {member.mesesActivos};
...
Object.assign(window, { Solicitar, ClaimTimeline, CLAIM_STEPS, stepIndex, NoElegible });
