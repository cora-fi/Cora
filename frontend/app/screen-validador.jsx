/* ============================================================
   Cora — Pantalla 8: Validador (detrás de rol)
   Bandeja de solicitudes por aprobar / rechazar.
   ============================================================ */
function Validador() {
  const claims = useAsync(() => coraService.getClaims(), []);
  const [busy, setBusy] = useState(null);
  const [toast, setToast] = useState(null);

  const pendientes = (claims.data || []).filter((c) => c.estado === 'enviado' || c.estado === 'en_validacion');
  const resueltos = (claims.data || []).filter((c) => c.estado !== 'enviado' && c.estado !== 'en_validacion');

  const attest = async (c, aprobar) => {
    setBusy(c.id + (aprobar ? 'a' : 'r'));
    try {
      const updated = await coraService.attestClaim(c.id, aprobar);
      claims.reload();
      setToast(aprobar
        ? (updated.estado === 'aprobado' || updated.estado === 'pagado'
            ? 'Aprobada. El fondo co-paga al hospital.'
            : `Tu aprobación quedó registrada (${updated.aprobaciones} de ${updated.aprobacionesNecesarias}).`)
        : 'Solicitud rechazada.');
      setTimeout(() => setToast(null), 3200);
    } catch (e) { setToast('No se pudo registrar.'); }
    setBusy(null);
  };
                  ...
                  <Card key={c.id} pad="var(--s3)">
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14, marginBottom: 16 }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div className="font-serif tnum" style={{ fontSize: 22 }}>{money(c.montoSolicitado)}</div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 28px', padding: '14px 0',
                      ...
                      <ChipFact icon="hourglass-high" label="En lista" value={`${c.diasEnLista} días`} flag={c.diasEnLista > 180} />
                      <ChipFact icon="map-pin" label="Ciudad" value={c.hospital.ciudad} />
                      <ChipFact icon="users-three" label="Aprobaciones" value={`${c.aprobaciones} de ${c.aprobacionesNecesarias}`} />
                      <ChipFact icon="calendar-blank" label="Recibida" value={fmtDate(c.fecha)} />
                    </div>

                    <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                      <Button variant="danger" icon="x" loading={busy === c.id + 'r'}
                        onClick={() => attest(c, false)}>Rechazar</Button>
                      <Button variant="primary" icon="check" loading={busy === c.id + 'a'}
                ...
                {resueltos.map((c, i) => (
                  <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14,
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      <span className="font-serif tnum" style={{ fontSize: 17 }}>{money(c.montoSolicitado)}</span>
                      <Badge status={c.estado} />
                    </div>
                  </div>
                ))}
...
Object.assign(window, { Validador, ChipFact });
