/* ============================================================
   Cora — Pantalla 2: Dashboard (home)
   Entrada escalonada + count-up en cifras clave.
   ============================================================ */
function Rise({ delay = 0, children, style }) {
  return <div className="rise" style={{ animationDelay: `${delay}s`, ...style }}>{children}</div>;
}
...
function Dashboard({ member, go }) {
  ...
  return (
    <div style={{ maxWidth: 1080 }}>
      ...
      {/* fila principal: cobertura + carencia */}
          ...
          <Card style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 220 }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span className="font-serif tnum" style={{ fontSize: 46, fontWeight: 500, color: 'var(--clay)' }}>{member.mesesActivos}</span>
                <span className="font-serif" style={{ fontSize: 22, color: 'var(--ink-2)' }}>/ {member.mesesCarencia} meses</span>
              </div>
            </div>
            <div>
          ...
          <Card>
            <StateWrap status={pool.status === 'ok' ? 'ok' : pool.status} error={pool.error} onRetry={pool.reload}
              ...
              {pool.data && <>
                ...
                <StatRow label="Generando rendimiento" value={<Money value={pool.data.colocadoEnRendimiento} />} accent="var(--forest)" />
                <StatRow label="Disponible para atenciones" value={<Money value={pool.data.disponible} />} />
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 13, color: 'var(--ink-2)', fontSize: 14 }}>
                  <span>{pool.data.miembros} miembros</span>
                  <span>{pool.data.claimsPagados} atenciones co-pagadas</span>
                </div>
              </>}
            </StateWrap>
          </Card>
              ...
              {yld.data && <>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 6 }}>
                  <div className="font-serif tnum" style={{ fontSize: 38, fontWeight: 500, color: 'var(--forest)' }}>
                    <Money value={yld.data.rendimientoAcumulado} count />
                  </div>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 13px',
                  ...
                  <span className="tnum">{yld.data.apyAprox}%</span> aprox. anual
                </div>
              </>}
            </StateWrap>
          </Card>
...
Object.assign(window, { Dashboard, Rise, StatRow });
