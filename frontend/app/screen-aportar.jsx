/* ============================================================
   Cora — Pantalla 3: Aportar + historial
   ============================================================ */
function Aportar({ member, onMemberChange }) {
  const cfg = window.CoraConfig.product;
  const [monto, setMonto] = useState(cfg.primaMensual);
  const [phase, setPhase] = useState('form'); // form | review | processing | done | error
  const [last, setLast] = useState(null);
  const hist = useAsync(() => coraService.getContributions(), []);
  ...
  const confirmar = async () => {
    setPhase('processing');
    try {
      const c = await coraService.contribute(Number(monto));
      setLast(c);
      setPhase('done');
      hist.reload();
      const m = await coraService.getMember();
      onMemberChange(m);
    } catch (e) {
      setPhase('error');
    }
  ...
  return (
    <div style={{ maxWidth: 920 }}>
      <Rise delay={0.02}><SectionTitle eyebrow="Tu aporte mensual"
        sub="Cada aporte sostiene el fondo común. Es lo que permite que, cuando a alguien le toca, haya con qué responder.">
        Aportar
      </SectionTitle></Rise>
            ...
            {phase === 'done' && (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <p style={{ color: 'var(--ink-2)', margin: '8px 0 4px' }}>
                  Sumaste <strong style={{ color: 'var(--ink)' }}>{money(last?.monto)}</strong> al fondo. Gracias por sostenerlo.
                </p>
              ...
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14,
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  ...
                  <div>
                    ...
                    <div className="font-mono" style={{ fontSize: 11.5, color: 'var(--ink-2)' }}>{fmtDate(c.fecha)} · {c.id}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <Badge status={c.estado} />
                  <span className="tnum font-serif" style={{ fontSize: 18, minWidth: 56, textAlign: 'right' }}>{money(c.monto)}</span>
                </div>
              </div>
            ))}
          </StateWrap>
...
Object.assign(window, { Aportar, ReviewRow });
